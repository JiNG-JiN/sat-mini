(function(Site){
	...
	MediaSlideshow = View.extend({
		...
		closeVideo: function() {
			if (this.slides[this.currentSlide]) this.slides[this.currentSlide].$('iframe').remove();
		}
	});
  ...
  MediaBoxes = View.extend({
	...
	MediaBox = View.extend({
		...
		initialize: function() {
			...
			this.render();
			if ( ! this.$el.hasClass('social') ) this.$('header h1').dotdotdot({ watch: 'window', tolerance: 2 });
		},
		render: function() {
			this.centerImage();
			if ( ! this.visible ) {
		...
		navToUrl: function(e) {
			...
			if (this.$el.hasClass('no_link')){
				return false;
			}

			if (clickedElement.is('img') && clickedElement.hasClass('loaded')) {
